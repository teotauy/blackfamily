...
- **Combined server** (`index.js`): Serves frontend + API together, but the root `package.json` is missing required deps (`express`, `cors`, `jsonwebtoken`, `sqlite3`). Use the backend server for development.
- **Frontend** (`index.html`, `js/app.js`, `css/style.css`): Vanilla HTML/CSS/JS. The API URL is hardcoded to the remote Render host in `js/app.js` line 5 (`API_BASE`).
...
Note: The frontend's `API_BASE` in `js/app.js` points to `https://blackfamilybackend.onrender.com/api`. To test against the local backend, you would need to change this to `http://localhost:5000/api`.
